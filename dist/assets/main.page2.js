...
async function apiRequest(code){
    try {
        const response = await axios_default().get(`https://restcountries.eu/rest/v2/name/${code}`);
...
async function apiRequestCountryBorders(code){
    ...
    try {
        const response = await axios_default().get(`https://restcountries.eu/rest/v2/alpha?codes=${code.join(';')}`);
...
function renderMainFlag(country){
    ...
    const resLangsMap = l.filter(i=> resLangs.includes(i.code)).map(i=>i.name);
